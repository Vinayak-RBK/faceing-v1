...
public class GuestUserSearchServiceImpl implements GuestUserSearchService {
	...
	private static final Logger logger = LoggerFactory.getLogger(GuestUserSearchServiceImpl.class);
...
//			return guestListRespDao;
		...
		logger.info("While searching all the Guest for User Id {} - {}", userId, guestListRespDao.getMsg());
		return guestListRespDao;
	...
	public GuestInfoListDao findGuestByGuestIdAndUserId(String guestId, String userId) {
		...
		if (user.isEmpty()) {
			...
			logger.info("While Finding Guest by Guest Id {} and User Id {} - {}", guestId, userId, wrongUserId);
			return guestListRespDao;
		}
		...
		if (!guestEntityList.get().isEmpty()) {
			...
			try {
				...
				logger.error(
						"Exception occurred during encryption While Finding Guest by Guest Id {} and User Id {} - {} ",
						guestId, userId, e1);
				return guestListRespDao;
			}
			...
			for (Guest ent : guestEntityList.get()) {
				if (FieldNames.SDK_ANURA.equals(enSdkType)) {
					...
					if (guestHealthAnuraDetail == null) {
						...
						logger.info("While Finding Guest by Guest Id {} and User Id {} - {}", guestId, userId,
								guestHealthRecordsNotFound);
						return guestListRespDao;
					} else {
	...
	public GuestInfoListDao getGuestDetailsByExcel(String userId, HttpServletResponse response) {
		...
		if (user.isEmpty()) {
			...
			logger.info("While getting Guest for sending data for Excel Download - {}", infoListDao.getMsg());
			return infoListDao;
		}
		try {
			...
			logger.error("Exception occurred While getting Guest for sending data for Excel Download - {} ",
					infoListDao.getMsg());
		}
	...
	public PagerResponseDao findAllGuestByUserId(PagerRequestDao pagerDao) {
		...
		try {
			...
			logger.error("Exception occurred during encryption While Finding Guest by User Id {} - {} ",
					pagerDao.getUserId(), e1);
			return respDao;
		}
		...
		if (totalUsersCount == 0) {
			...
			logger.info("While Finding Guest by User Id {} - {}", pagerDao.getUserId(), respDao.getMsg());
			return respDao;
		}
