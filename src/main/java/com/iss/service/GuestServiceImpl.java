...
public class GuestServiceImpl implements GuestService {
	...
	public Response addGuest(GuestHealthInfoDao guestInfoDao, String dateTimeFormat) {
		...
		try {
			...
			if (guestEnt != null) {
				...
				if (FieldNames.SDK_ANURA.equals(decSdkType)) {
					...
					if (guestHealthAnuraDetail != null) {
						...
						logger.error("while adding Guest - {}", addGuestSuccess);
						return resp;
					} else {
						...
						logger.error("while adding Guest - {}", addGuestFail);
						return resp;
					}
				}
			...
			resp.setSuccess(Boolean.toString(false));
			logger.error("Exception occurred while adding guest - {}", e);
			return resp;
		}
		return resp;
	...
	public Response removeGuest(String guestId, String userId, String dateTimeFormat) {
		...
		if (guestEntityList.get().isEmpty()) {
			...
			logger.error("while removing Guest - {}", guestNotExists);
			return resp;
		} else {
			...
			try {
				...
				resp.setSuccess(Boolean.toString(false));
				resp.setMsg(someThingWentWrong + e);
				logger.error("Exception occured while removing Guest - {}", e);
				return resp;
			}
			...
			if (guest == null) {
				...
				logger.error("while removing Guest - {}", unableToRemoveGuest);
				return resp;
			} else {
				resp.setSuccess(Boolean.toString(true));
				resp.setMsg(removeGuestSuccess);

				logger.error("while removing Guest - {}", removeGuestSuccess);
				return resp;
			}
		}
	}
